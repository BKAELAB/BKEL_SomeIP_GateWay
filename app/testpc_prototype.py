...
def crc8_xor(data: bytes) -> int:
    """Prototype CRC8 placeholder. Replace with project CRC rule later."""
    crc = 0
    for b in data:
        crc ^= b
    return crc & 0xFF
...
class FrameCodec:
    ...
    def try_decode(buffer: bytearray) -> Optional[Frame]:
        ...
        return Frame(sid=sid, data_type=data_type, payload=payload, cid_raw=cid_raw, crc8=recv)
...
class PrototypeApp:
    def __init__(self, root: tk.Tk):
        ...
        self.pending_diag: Dict[int, int] = {}  # seq -> sid
    ...
    def default_payload_for_sid(sid: int) -> bytes:
        defaults = {
            0x10: b"\x02",  # toggle
            0x11: b"\x01",  # reset
            0x12: b"\x00\x00\x00\x00\x00",  # read + padding
            0x13: b"\x32\x0A",  # duty=50, period/freq=10
            0x20: b"\x00\x00",
            0x21: b"\x00\x00",
            0x22: b"\x00\x00",
            0x23: b"\x00\x00",
    ...
    def _handle_frame(self, frame: Frame) -> None:
        ...
        if frame.sid in DIAG_SIDS:
            human = self._diag_to_text(frame.sid, frame.payload)
            self._append_log(f"[DIAG] {human}")

    def _add_services_from_brief(self, mcu: McuInfo, brief: str) -> None:
        # Prototype parser: accepts patterns like "0x10,0x22,0x24"
        found = set()
    ...
    def _diag_to_text(sid: int, payload: bytes) -> str:
        ...
        if sid in {0x24, 0x25, 0x26} and payload:
            state = "High" if payload[0] else "Low"
            return f"SID=0x{sid:02X}: PinState={state}"
