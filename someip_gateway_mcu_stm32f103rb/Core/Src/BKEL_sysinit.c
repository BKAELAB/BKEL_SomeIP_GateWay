...
void system_init(void) {
	...
	/* 시스템 클럭 동기화 설정
	 * UART 전송 시 Putty에서 글자 깨짐 발생
	 * BKEL_CLK_Init()에서 HSE 8MHz -> PLL x9를해서 72MHz만듬
...
static void BKEL_CLK_Init(void) {
	...
	while (!(RCC->CR & (RCC_CR_HSERDY_FLAG)))
		;    	// HSE 오실레이터가 안정되었음을 나타내기 위해 하드웨어에 의해 설정
	...
	while ((RCC->CFGR & (RCC_CFGR_SWS_CLEAR)) != (RCC_CFGR_SWS_PLL_STATUS))
		;	// 시스템 클럭이 실제로 PLL로 바뀌었는지 확인
}
...
static void BKEL_SPI_Init() {
	...
	SPI2->CR1 &= ~(SPI_CR1_DFF);	// DataSize = 8BIT(DFF=0)
